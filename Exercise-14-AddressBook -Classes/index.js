...
//   // const addressBooks =[{'pid':10,'name':'Modi'},{'pid':10,'name':'Jassu'}];
  // const addressBooks = [{ pid: 10, friends: [11, 14] }, { pid: 11, friends: [12, 13] }
  // ];

  // for (let contactInfo of contactList) {
    const contact = new Contact(PersonDOM.getPid(), PersonDOM.getPname(),
    PersonDOM.getJob(), 
    PersonDOM.getHno(),
    PersonDOM.getStreet(),
    PersonDOM.getCity());
  ...
  for (let contact of Contacts) {
    ele.innerHTML = ele.innerHTML + '<option value="' + contact.pid + '">' + contact.pname + '</option>';
  }
